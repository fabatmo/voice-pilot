...
class DictationManager: ObservableObject {
    ...
    /// Called on global middle-mouse, Ctrl+Shift+S, or Ctrl+Return to submit dictation.
    /// Name kept for historical compatibility.
    ...
    // MARK: - Global CGEventTap (middle-mouse + Ctrl+Shift+S, consumed so they never reach the target app)
    ...
    private func attemptEventTap() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                // Middle-mouse — consume and submit (debounced)
                ...
                // Ctrl+Shift+S — consume and submit (debounced)
                if type == .keyDown {
    ...
    // MARK: - Ctrl+Return (observe-only, non-consuming NSEvent)
